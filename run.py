...
# from test_pipeline import test_fastqc, test_kallisto
...
#             function_to_convert_notebook(**eda_cfg)
    ...
    if 'test' in targets:
        ...
        with open('config/test-params.json') as fh:
            ...
            pass_data, cut_data = fastqc([fq_1,fq_2])
            print(pass_data)
            
            # cutadapt
            cut_fastq_files = cutadapt(pass_data)
  
            print("Running fastqc PART 2 (To reevaluate after data has been trimmed) from main:")
            pass_cut_data, fail_cut_data = second_fastqc()
            # kallisto:
            kallisto(pass_cut_data)
            # eda then report here:
#             with open('config/eda-params.json') as fh:
#                 eda_cfg = json.load(fh)
